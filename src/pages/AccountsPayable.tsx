...
const AccountsPayable = () => {
  ...
  const fetchAccounts = async () => {
    try {
      ...
      showError('Erro ao buscar contas a pagar: ' + error.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.description || !formData.amount || !formData.due_date || !formData.supplier_name) {
  ...
  const handleMarkAsPaid = async (account: AccountPayable) => {
    try {
      ...
      showError('Erro ao marcar conta como paga: ' + error.message);
    }
  };
  ...
  return (
    <div className="p-6 space-y-6 bg-gray-900 min-h-screen">
      <div className="flex justify-between items-center">
        <h1 className="text-3xl font-bold text-gray-100">Contas a Pagar</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Card className="bg-gradient-to-br from-red-900/40 to-red-800/20 border-red-700">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-red-400">Total a Pagar</CardTitle>
          </CardHeader>
          <CardContent>
              ...
              {loading ? (
                ...
                filteredAccounts.map((account) => (
                  <TableRow key={account.id} className="border-gray-700 hover:bg-gray-700/50">
                    ...
                    <TableCell className="text-gray-100">{account.supplier_name}</TableCell>
